...
class HanoiTower:
    ...
    def is_available(self, tower_from: str, tower_to: str) -> bool:
        ...
        if tower_from not in ("1", "2", "3") or tower_to not in ("1", "2", "3"):
    ...
    def move_circles(self) -> None:
        ...
        while True:
            ...
            if self.is_available(tower_from, tower_to):
                ...
                self._count += 1
                self.print_towers()
                if (not self._towers[0] and not self._towers[1]) or (not self._towers[0] and not self._towers[2]):
                    print(f"Победа! Шагов совершено: {self._count}. "
                          f"\nМинимально возможное количество шагов: {2**self._circle_amount - 1}.")
                    return
